...
void *recvMesgAndSendToAll(void* connectSockFd) {
	...
	{
		int temp = recv(*(int *) connectSockFd, start, sizeof(struct mesgPackage), 0);
		start+= temp;
		recvbytes-=temp;
		recvd+=temp;
	}
	...
	for (int i = 1; i <= num_hosts; i++) {
		...
		if (strcmp(hostnames[i], localhostname) != 0 && i != 1 && recvMesg.pid[i]==0) {
...
void sendMessage(struct mesgPackage mesg, char* dest_hostname, int port) {
	...
	if(-1 == send(sock, (struct mesgPackage*) &mesg, sizeof(struct mesgPackage), 0))
	{
